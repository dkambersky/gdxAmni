...
/**
 * <p>
 * The champion method and its supporting operations.
 * <p>
 * <p>
 ...
 * </p>
 * 
 * @see <a href="http://developer.riotgames.com/api/methods#!/291">Developer
 *      site</a>
 * 
 * @author Enigma
 */
public class ChampionMethod extends Method {
	...
	/**
	 * Returns a list of all available or free champions and their accompanying
	 * information in the given region.
	 * 
	 * @param region
	 *            The game region (NA, EUW, EUNE, etc.)
	 * @param free
	 *            Whether or not to return the free champions only.
	 * @return A list of all available or free champions.
	 * @throws RegionNotSupportedException
	 *             If the region is not supported by the method.
	 * @throws RiotApiException
	 *             If there was an exception or error from the server.
	 */
	public List<Champion> getChampions(Region region, boolean free)
			throws RiotApiException {
		Response response = getMethodResult(region,
				createArgMap("freeToPlay", String.valueOf(free)));
		...
		for (int n = 0; n < csArray.size(); n++) {
			...
			Champion c = new Champion(api, region, cObj.getString("name"),
					cObj.getLong("id"), cObj.getInt("attackRank"),
					cObj.getInt("magicRank"), cObj.getInt("defenseRank"),
					cObj.getInt("difficultyRank"), cObj.getBoolean("active"),
					cObj.getBoolean("freeToPlay"),
					cObj.getBoolean("botMmEnabled"),
					cObj.getBoolean("botEnabled"),
					cObj.getBoolean("rankedPlayEnabled"));
			cs.add(c);
		}
		return cs;
	}
	...
	/**
	 * Returns a list of all available champions and their accompanying
	 * information in the given region.
	 * 
	 * @param region
	 *            The game region (NA, EUW, EUNE, etc.)
	 ...
	 *             If there was an exception or error from the server.
	 */
	public List<Champion> getAllChampions(Region region)
			throws RiotApiException {
		return getChampions(region, false);
	}

	/**
	 * Returns a list of all free champions (10 total) and their accompanying
	 * information in the given region.
	 * 
	 * @param region
	 *            The game region (NA, EUW, EUNE, etc.)
	 ...
	 *             If there was an exception or error from the server.
	 */
	public List<Champion> getFreeChampions(Region region)
			throws RiotApiException {
		return getChampions(region, true);
	}

	/**
	 * Returns a list of all disabled champions and their accompanying
	 * information in the given region.
	 * 
	 * @param region
	 *            The game region (NA, EUW, EUNE, etc.)
	 ...
	 *             If there was an exception or error from the server.
	 */
	public List<Champion> getDisabledChampions(Region region)
			throws RiotApiException {
	...
	/**
	 * Fills missing information in a champion if required, i.e., if the
	 * champion name or champion ID is missing.
	 * 
	 * @param region
	 *            The game region (NA, EUW, EUNE, etc.)
	 * @param champion
	 *            The champion to fill.
	 * @return <code>true</code> if information the champion was filled,
	 ...
	 *             If there was an exception or error from the server.
	 */
	public boolean fillChampion(Champion champion, Region region)
			throws RiotApiException {
		List<Champion> champions = getAllChampions(region);
		Champion newChampion = null;
		for (Champion c : champions)
			if ((champion.getId() >= 0 && c.getId() == champion.getId())
					|| c.getName().equals(champion.getName())) {
				newChampion = c;
				break;
			}
